...
@export_range(0.1, 100.0, 0.1) var minimum_camera_size := 5.5
@export_range(0.1, 100.0, 0.1) var maximum_camera_size := 12.0
...
func begin_shuffle() -> void:
	_kill_tween()
	var target_size := minf(
		maxf(camera.size * shuffle_zoom_multiplier, minimum_camera_size),
		maximum_camera_size
	)
...
func settle_on_board() -> void:
	...
	if tiles.is_empty():
		return
	var target := _calculate_target(tiles)
	_kill_tween()
...
func _on_board_built(_tiles: Array[Node3D]) -> void:
	_kill_tween()
	camera.global_position = _default_camera_position
	camera.size = _default_camera_size


func _calculate_target(tiles: Array[Node3D]) -> Dictionary:
	...
	target_size = clampf(target_size, minimum_camera_size, maximum_camera_size)
